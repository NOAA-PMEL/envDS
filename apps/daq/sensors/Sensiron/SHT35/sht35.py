...
class SHT35(Sensor):
    ...
    metadata = {
        "attributes": {
            # "name": {"type"mock1",
            "make": {"type": "string", "data": "Sensiron"},
            "model": {"type": "string", "data": "SHT35"},
            "description": {
                "type": "string",
    ...
    def __init__(self, config=None, **kwargs):
        super(SHT35, self).__init__(config=config, **kwargs)
        self.data_task = None
        self.data_rate = 1
    ...
    def configure(self):
        ...
        self.config = SensorConfig(
            make=SHT35.metadata["attributes"]["make"]["data"],
            model=SHT35.metadata["attributes"]["model"]["data"],
    ...
    async def handle_interface_data(self, message: Message):
        await super(SHT35, self).handle_interface_data(message)
            ...
            try:
                ...
                try:
                    ...
                    if hexdata and "0x"in hexdata:
                        ...
                        record["variables"]["temperature"]["data"] = round(cTemp,2)
                        record["variables"]["rh"]["data"] = round(rh,2)
                        return record
...
async def main(server_config: ServerConfig = None):
    ...
    logger = logging.getLogger(f"Sensiron::SHT35::{sn}")

    logger.debug("Starting SHT35")
    inst = SHT35()
    # print(inst)
    # await asyncio.sleep(2)
    inst.run()
    ...
    while do_run:
        logger.debug("sht35.run", extra={"do_run": do_run})
        await asyncio.sleep(1)
