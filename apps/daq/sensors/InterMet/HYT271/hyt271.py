...
class HYT271(Sensor):
    ...
    def configure(self):
        ...
        for name, setting in HYT271.metadata["settings"].items():
...
async def main(server_config: ServerConfig = None):
    ...
    logger = logging.getLogger(f"InterMet::HYT271::{sn}")

    logger.debug("Starting imet-HYT271")
    inst = HYT271()
    # print(inst)
    # await asyncio.sleep(2)
    inst.run()
    ...
    while do_run:
        logger.debug("InterMet.HYT271.run", extra={"do_run": do_run})
        await asyncio.sleep(1)
