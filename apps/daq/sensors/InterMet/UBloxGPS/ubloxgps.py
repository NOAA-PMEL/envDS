...
class UBloxGPS(Sensor):
    """docstring for UBloxGPS."""

    metadata = {
        "attributes": {
            # "name": {"type"mock1",
            "make": {"type": "string", "data": "InterMet"},
            "model": {"type": "string", "data": "UBloxGPS"},
            "description": {
                "type": "string",
                "data": "UBlox GPS Receiver",
            },
            "tags": {
                "type": "char",
        ...
        "variables": {
            ...
            "gps_date": {
                "type": "str",
                "shape": ["time"],
                "attributes": {"long_name": {"type": "string", "data": "GPS Date"}},
            },
            "gps_time": {
                "type": "str",
                "shape": ["time"],
                "attributes": {"long_name": {"type": "string", "data": "GPS Time"}},
            },
            "latitude": {
                "type": "float",
                "shape": ["time"],
            ...
            "num_sat": {
                ...
                "shape": ["time"],
                "attributes": {
                    "long_name": {"type": "char", "data": "Number of Satellites"},
    ...
    def __init__(self, config=None, **kwargs):
        super(UBloxGPS, self).__init__(config=config, **kwargs)
        self.data_task = None
        self.data_rate = 1
        # self.configure()
    ...
    def configure(self):
        ...
        for name, setting in UBloxGPS.metadata["settings"].items():
        ...
        self.config = SensorConfig(
            make=UBloxGPS.metadata["attributes"]["make"]["data"],
            model=UBloxGPS.metadata["attributes"]["model"]["data"],
    ...
    async def handle_interface_data(self, message: Message):
        await super(UBloxGPS, self).handle_interface_data(message)
...
async def main(server_config: ServerConfig = None):
    ...
    logger = logging.getLogger(f"InterMet::UBloxGPS::{sn}")

    logger.debug("Starting imet-UBloxGPS")
    inst = UBloxGPS()
    # print(inst)
    # await asyncio.sleep(2)
    inst.run()
    ...
    while do_run:
        logger.debug("InterMet.UBloxGPS.run", extra={"do_run": do_run})
        await asyncio.sleep(1)
