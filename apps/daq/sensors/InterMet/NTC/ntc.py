...
class NTC(Sensor):
    ...
    metadata = {
        "attributes": {
            ...
            "model": {"type": "string", "data": "NTC"},
            "description": {
                "type": "string",
                "data": "Glass Bead NTC Thermistor, Shibaura PSB-S5, Calibrated by InterMet",
            },
            "tags": {
                "type": "char",
        ...
        "variables": {
            ...
            "resistance": {
                ...
                "shape": ["time"],
                "attributes": {
                    "long_name": {"type": "char", "data": "Thermistor resistance"},
                    "units": {"type": "char", "data": "ohm"},
                },
            },
        },
        "settings": {
    ...
    def __init__(self, config=None, **kwargs):
        super(NTC, self).__init__(config=config, **kwargs)
        self.data_task = None
        self.data_rate = 1
        # self.configure()
    ...
    def configure(self):
        ...
        for name, setting in NTC.metadata["settings"].items():
        ...
        self.config = SensorConfig(
            make=NTC.metadata["attributes"]["make"]["data"],
            model=NTC.metadata["attributes"]["model"]["data"],
    ...
    async def handle_interface_data(self, message: Message):
        await super(NTC, self).handle_interface_data(message)
            ...
            try:
                ...
                try:
                    ...
                    if len(parts) == 3 and parts[0] == self.sensor_id:
                        ...
                        record["variables"]["temperature"]["data"] = round(float(parts[1])/100.0,2)
                        record["variables"]["resistance"]["data"] = float(parts[2])
                        return record
...
async def main(server_config: ServerConfig = None):
    ...
    logger = logging.getLogger(f"InterMet::NTC::{sn}")

    logger.debug("Starting imet-NTC")
    inst = NTC()
    # print(inst)
    # await asyncio.sleep(2)
    inst.run()
    ...
    while do_run:
        logger.debug("InterMet.NTC.run", extra={"do_run": do_run})
        await asyncio.sleep(1)
