...
# class envdsDashboard(envdsBase):
#     """docstring for envdsDashboard."""
...
#     def configure(self):
#         super(envdsDashboard, self).configure()
...
#             self.id.app_env_id,
#         ]
#         return (envdsDashboard.ID_DELIM).join(parts)
...
#                 id_parts = source_id.split(envdsDashboard.ID_DELIM)
...
#         super(envdsDashboard, self).set_routes(enable)
...
async def main(server_config: ServerConfig = None):
    ...
    logger = logging.getLogger("envds-dashboard")
    ...
    config = uvicorn.Config(
        ...
        root_path="/envds/dashboard",
        # ws="websockets",
        # log_config=dict_config,
    )

    print(f"*** here: {server_config}")
    await asyncio.sleep(5)
    server = uvicorn.Server(config)
    # server = uvicorn.Server("main:app")
